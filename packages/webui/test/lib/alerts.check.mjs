...
describe("pushAlert — dedup", () => {
    ...
    test("dedup survives ring wrap (no collateral increment on stale index)", () => {
        // Regression for V01 Finding 1: the _dedupIndex Map stores
        // alert object references (not buffer indices) so a wrap
        // does not corrupt unrelated entries in the ring buffer.
        for (let i = 0; i < 100; i++) {
            alerts.pushAlert({
                level: "info",
                msg: "m" + i,
...
describe("pushAlert — event-stream emission (B01 dependency)", () => {
    ...
    test("audit-write failure does not break the alert flow", async () => {
        // The events.js module is dynamic-imported by alerts.js on first
        // push; the missing-module case must not propagate into pushAlert.
        const a = alerts.pushAlert({
            level: "error",
            msg: "no-audit",
            src: "test",
    ...
    test("audit write carries alert payload (id, msg, count, sessionId, data)", async () => {
        // Regression for V01 Finding 2: alert fields must be wrapped
        // inside `payload: {...}` per the events.js contract —
        ...
        // import it here too to clear the in-memory seq / hash state
        // (otherwise a previous test's chain head leaks in).
        ...
        try {
            ...
            // test/lib/events.test.js handles its own cleanup.)
        }
    });
});
