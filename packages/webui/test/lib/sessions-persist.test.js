...
// Unit tests for server/lib/sessions.js persistence boundaries (PR #55
// review point 5).
//
// saveSessions must use tmp-file + atomic rename, and loadSessions must
// never treat parse failures as an empty DB — otherwise the next load-
// modify-save wipes the user's real data. Corruption handling: quarantine
// copy + actionable error, original file never touched by load.
//
// These tests pin:
...
//
// Module-load order: SESSIONS_DB is computed by config.js at import
// time from MCODE_WEBUI_SESSIONS_DB, so the env var is set BEFORE the
// SUT's dynamic import (node --test gives each file its own process).
