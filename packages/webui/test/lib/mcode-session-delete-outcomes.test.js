// webui/test/lib/mcode-session-delete-outcomes.test.js
...
//                                         failure (rolled back)
//
// Fixture strategy mirrors test/lib/mcode-session-delete.test.js (real
...
// Same gating as test/lib/mcode-session-delete.test.js: (a) a working sqlite3
...
describe("deleteMcodeSessionFromDb — lock conflict aborts (no fake success)", { skip: DB_FIXTURE_SKIP }, () => {
  test("SQLITE_BUSY from a concurrent writer → {ok:false, reason:'db_error'}, row survives, then clean retry succeeds", () => {
    ...
    try {
      ...
      // delete attempt (the 5s busy_timeout waits, then fails).
