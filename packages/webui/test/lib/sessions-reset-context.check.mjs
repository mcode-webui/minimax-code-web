// webui/test/lib/sessions-reset-context.check.mjs
...
// /clear / delete fan-out / protocol activate-session). It must drop
// cs.running to the idle shape (byte-mirrored from the runners'
// finalize() + makeClientState()) and set context.thinkingStatus =
// "Idle". Without this, a mid-run switch leaves running.active=true
// parked in the client state — and commands.js#handleCmdCommand("/new")
// REFUSES to create a session while cs.running.active, so a stale
// claim permanently blocks /new.
//
// Two scoped boundaries pinned here as NEGATIVE assertions:
//   - cs.chat is NOT touched (no ▍ cursor stripping): every caller has
//     already replaced or cleared the chat — stripping here could
//     corrupt the TARGET session's lines. Contrast chat.js#resetThinkingClaim
//     (same-session terminal failure), which DOES strip.
//   - context.lastUsageAt IS nulled (session-change semantics: the target
//     session has no observed usage yet) — again contrasting
//     resetThinkingClaim, which keeps it because the session is the same.
//
// Test strategy: NO mocks — this file imports the REAL lib/sessions.js
// directly (it is a pure module: only config.js constants at import time,
// zero IO).
...
// with IDLE_RUNNING in test/routes/chat-failed-send.check.mjs.
