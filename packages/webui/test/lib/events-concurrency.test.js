...
//
// What's covered here vs the existing test/lib/events.test.js +
// test/lib/events-hash.test.js:
...
describe("events concurrency (D01) — reset stability", () => {
  ...
  test("reset alone preserves on-disk file (deleteFile is opt-in)", () => {
    ...
    // the file is untouched. (deleteFile must remain opt-in so plain
    // resets never destroy evidence.)
  ...
  test("simulated fresh start: write+rm file+reset → next append starts at seq=1", () => {
    ...
    // test/lib/events.test.js; this variant keeps the explicit rm form
