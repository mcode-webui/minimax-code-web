// webui/test/lib/sqlite-resolver-c01.test.js
// Lease D01 — Coverage gap fillers for server/lib/sqlite-resolver.js (C01 round 7).
//
// What's covered here vs the existing test/lib/sqlite-resolver.test.js:
...
//       7. MCODE_WEBUI_RESOLVER_JSON env override (used by tests on some
...
// / _getBetterSqlite3Candidates, which together cover most of
// sqlite-resolver.js's resolver code.
...
// placeholder BEFORE sqlite-resolver.js (and thus config.js) is imported
// below — config.js freezes MCODE_CMD at import time (env MCODE_CMD >
// repo layout mcode.cmd > ~/.minimax-code/mcode.cmd > "mcode"), so a
// per-test mutation in a before() hook would be too late.
// sqlite-resolver.js only emits MCODE_CMD-derived better-sqlite3
// candidates when the value is NOT the placeholder (sqlite-resolver.js:122),
// so this pin (a) suppresses tier 2 on machines where an mcode.cmd
// layout exists and (b) neutralizes a real-binary MCODE_CMD inherited
// from the developer's shell — either would let the host load its
// better-sqlite3 and flip the D01 not-loaded tests to ok:true. Every
// other describe in this file passes mcodeCmd explicitly, so the pin
// is inert outside D01. Restored in the file-level after() below.
const SAVED_MCODE_CMD = process.env.MCODE_CMD;
process.env.MCODE_CMD = "mcode";

before(async () => {
  db = await import(dbUrl);
  del = await import(delUrl);
});
after(() => {
...
// ---------------------------------------------------------------------------
describe("sqlite-resolver.js — C01 MCODE_BETTER_SQLITE3 env priority", () => {
...
// ---------------------------------------------------------------------------
describe("sqlite-resolver.js — C01 MCODE_CMD path reverse", () => {
...
// ---------------------------------------------------------------------------
describe("sqlite-resolver.js — C01 built-in fallback ordering", () => {
...
describe("sqlite-resolver.js — D01 deleteMcodeSessionFromDb pre-flight gates", () => {
  ...
  //           a failed env override: sqlite-resolver.js:165-171 records the failure
  ...
  //           of this file (see SAVED_MCODE_CMD) → sqlite-resolver.js:124 emits no
  //           MCODE_CMD-derived candidates.
  //   tier 3  MCODE_WEBUI_RESOLVER_JSON → nonexistent temp path →
  //           _loadUserResolverConfig fails open to [] (sqlite-resolver.js:90-92),
  ...
  //           (sqlite-resolver.js:121 default param) and follows $HOME on POSIX /
  ...
  // flag (_McodeBetterSqlite3Failed, sqlite-resolver.js:41/175), so the second test
