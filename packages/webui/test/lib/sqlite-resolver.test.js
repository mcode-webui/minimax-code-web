// webui/test/lib/sqlite-resolver.test.js
// Tests for the better-sqlite3 candidate path resolver (v1.0.1
// round 4, C01 round 7).
//
// Round 4 added a candidates list with priority: env override >
// MCODE_CMD derived > dev layout fallback. C01 (round 7) added
// install-layout scenarios covering the
...
describe("sqlite-resolver.js — better-sqlite3 resolver candidates (v1.0.1 round 4)", () => {
  ...
  test("candidates count: env (1) + MCODE_CMD (0 or 1) + dev layout (1) = 1, 2, or 3", () => {
    // v2.3 (bundle-convergence): the math changed because two monorepo
    //   layouts were added as the lowest-priority tail. The exact
    //   invariant we still test is "env adds exactly 1 candidate at the
    //   front, regardless of the rest of the layout" — which is what
    //   makes the test name's count claim a comment, not a binding
    //   assertion. Without env, the count is N when MCODE_CMD is "mcode"
    //   and N+1 (or N+2) when MCODE_CMD is a real path; with env, add 1.
    delete process.env.MCODE_BETTER_SQLITE3;
...
describe("sqlite-resolver.js — C01 install-layout scenarios (user resolver config + tier ordering)", () => {
  let savedEnvPath;
