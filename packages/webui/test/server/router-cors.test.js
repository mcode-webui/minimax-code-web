// webui/test/server/router-cors.test.js
...
// The CORS and Origin/CSRF gates moved out of `router.js` into `lib/gates.js` so that a
// second HTTP layer can run the same chain. These guards are about the *request path*,
// not about one file, so they read both: pinning them to a single path would go red on
// every move and tempt someone to weaken them instead of following the code.
const routerSource = [
...
describe("router — Gate 1b Origin/CSRF gate (v2, PR #55 review point 1)", () => {
  test("mutating request with untrusted Origin is 403'd before other gates", () => {
    ...
    // Slice only the gate block itself, ending at the statement that opens the
    // locality-aware part of the chain — `const local = isLocalRequest(req)` belongs to
    // gates 2+ which legitimately consult locality, so it must stay outside the slice.
    const gateEnd = routerSource.indexOf("const local = isLocalRequest(req)", gate1b);
