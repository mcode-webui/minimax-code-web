// webui/test/server/server-startup.test.js
// Smoke test for server.js bootstrap. Catches ESM load-time blockers
// that unit tests don't catch — they import modules one at a time,
// never exercising the real server.js bootstrap path.
//
// Strategy: spawn `node server.js`, wait briefly, then SIGTERM. If
// startup failed, the node process exits early with an ESM error
...
//
// v2 (Lease C08, §AP1): assert that the 14-line ASCII token box is
// GONE from stdout on first-ever boot (settings.json absent + no
// TOKEN env). The SSE `token.first_run` event is what the UI now
// uses — see state-bus.js + auth.js#markFirstRunNotified. The
// `MCODE_WEBUI_TOKEN_STDOUT=1` fallback is verified to print a single
// neutral line that contains the persist path but NEVER the raw
// 32-hex token.
