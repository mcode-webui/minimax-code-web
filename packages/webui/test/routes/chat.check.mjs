// webui/test/routes/chat.check.mjs
...
// The flag-free production-wiring describe block lives in
// test/integration/chat-wiring.test.js.
...
describe("handleStop", () => {
  ...
  test("session/cancel accepted: cancelled=true and no immediate hard kill", async () => {
    // The default mock for mcode-rpc.cancelSession reports the notification as sent.
    const cid = "cid-1";
    const cs = makeClientState();
    cs.workspace = { dir: "/ws-X", branch: null, tree: null };
    ...
    assert.equal(body.cancelled, true, "the notification reached the engine");
    assert.equal(body.hardKilled, false, "no SIGKILL when the engine was asked to cancel");
