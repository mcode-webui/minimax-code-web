// webui/test/routes/export.check.mjs
...
//      test/helpers/_setup.js#withDecisions (approve for happy paths, an
//      explicit decline test for the 403)
//
// Test strategy:
//   - Use setupMocks to mock lib/sessions.js (webui chat store) and
//     lib/sqlite-resolver.js (mcode better-sqlite3 unavailable —
//     export.js's mcode-transcript enrichment depends on
//     getMcodeBetterSqlite3 returning null to exercise the
//     mcode_unavailable branch). lib/authorize.js is NOT mocked —
//     decisions are injected through its real pending registry (see
//     exportApproved / exportDeclined below).
...
// (2026-09-20 webui-rigor-fix — no auto-approve): exportApproved
// injects a user approval, exportDeclined a user rejection.
...
before(async (t) => {
  ...
  // Mock lib/sqlite-resolver.js so getMcodeBetterSqlite3 returns null
  // (mcode db unavailable in tests). This is the realistic scenario —
  // the production environment may or may not have better-sqlite3
  // installed in the expected location, and the export route must
  // continue to serve when it doesn't. After the 2026-09-25
  // webui-bundle-convergence split, the resolver chain lives in
  // sqlite-resolver.js (lib/mcode-session-delete.js holds the
  // deleteMcodeSessionFromDb loop, which export.js does not call
  // directly — so a single mock on the resolver is enough here).
