// webui/test/routes/sessions-switch.check.mjs
...
//
// Test strategy: setupMocks for the webui module surface; the
// better-sqlite3 module surface is mocked with a FAKE class keyed by
// SQL string — legacy probe SQLs always throw (mirroring the real DB's
// schema drift), the v2 data_json probe returns fixture rows.
//   - lib/sqlite-resolver.js exports getMcodeBetterSqlite3 — imported
...
// That same property doubles as the "export.js behavior unchanged"
// guard: export's legacy-only probe set still reports
// mcode_unavailable against this fake, exactly as it does against the
// real DB today.
...
// Mutable per-test knobs (the sqlite-resolver mock reads these at call time).
let _fakeDbOpts = {};
let _acpTitleCalls = 0;

// Isolate the audit stream — switch + export append hash-chain events
// that must never land in the operator's real
// ~/.mcode-webui/events.ndjson.
// v2 (2026-09-20 fork-preview fix): ALSO pin MCODE_RUNTIME_DB to a
// scratch file this check CREATES. readMcodeTranscript gates on
// existsSync(dbPath) BEFORE better-sqlite3 is ever consulted (reason
// "mcode_db_not_found"), and sessions.js passes dbPath from config.js's
// MCODE_RUNTIME_DB. The placeholder's CONTENT is irrelevant
// (better-sqlite3 is module-mocked below; the fake never reads the
// file) — existsSync just needs it to exist. Must be set BEFORE the
// first SUT import: config.js evaluates MCODE_RUNTIME_DB at
// module-load time.
let _tmpEventsDir;
let _tmpDbDir;
before(async (t) => {
  ...
  // sqlite-resolver.js fake: legacy probes always miss, v2 probe serves
  //   _fakeDbOpts. transcript.js (getMcodeBetterSqlite3) and
  //   session-tree.js (getMcodeBetterSqlite3) both import from here.
