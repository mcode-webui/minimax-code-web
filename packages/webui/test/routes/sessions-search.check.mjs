// webui/test/routes/sessions-search.check.mjs
// Unit tests for handleSearchSessions — Lease C05 cross-workspace search.
//
// CAPABILITIES.md §7 "Cross-workspace session search ⚠" is fixed to ✅
// by adding `GET /api/sessions/search`. Unlike the in-memory sidebar
// filter of old, the new handler surfaces sessions stored under a
// different `workspace` field.
//
// Test strategy:
...
//     fires for real on every search (no test-mode auto-approve, per
//     the 2026-09-20 webui-rigor-fix); the searchApproved helper
//     drives the real decision path via withDecisions.
