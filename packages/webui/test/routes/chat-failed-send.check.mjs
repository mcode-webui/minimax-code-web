// webui/test/routes/chat-failed-send.check.mjs
...
// missing, session/load throw, exec resolveMcodeSpawn fail-closed)
// never reaches the streaming runners' finalize() — finalize() is the
// ONLY place that returned cs to its at-rest thinking shape, so the
// pushed state kept claiming an active run and the context panel's
// 思考中 indicator never cleared.
//
// The route's failed branch (§AP3, lease B02) must:
//   1. Reset the thinking claim (running/thinkingStatus/thinkingDuration/
//      tps + strip the ▍ streaming cursor) so the terminal pushStateFor
//      lands an at-rest state.
...
// (test/helpers/_setup.js overrides.mcodeAcp) — mock.module() cannot be
...
describe("handleStop — zombie run claim", () => {
  ...
  test("live child present: handleStop does NOT touch cs.running (finalize owns it)", async () => {
    ...
    const body = JSON.parse(res._body);
    assert.equal(body.cancelled, true, "the cancel notification was sent");
    assert.equal(body.hardKilled, false, "no immediate SIGKILL once the engine is asked");
    // Contract pin: with a live child the abort rejects the in-flight
    // prompt and the runner's own finalize() performs the terminal
    // reset — handleStop must not race it by resetting early.
    assert.equal(
      cs.running.active,
      true,
