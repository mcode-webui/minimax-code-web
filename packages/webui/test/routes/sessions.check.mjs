// webui/test/routes/sessions.check.mjs
...
// deleteMcodeSessionFromDb, which hard-requires a loadable
// better-sqlite3 through sqlite-resolver.js's own resolver chain
// (mcode's bundled native module). Probe both preconditions via
// the production resolution faces (config.SQLITE3_BIN detection +
// getMcodeBetterSqlite3) and skip with the concrete reason when
// either is missing. Neither config.js nor sqlite-resolver.js is in
// setupMocks' mock surface, so importing them here is safe.
const _sqlite3BinProbe = (await import(absPath("lib/config.js"))).SQLITE3_BIN;
const _betterSqlite3Probe = (await import(absPath("lib/sqlite-resolver.js"))).getMcodeBetterSqlite3;
