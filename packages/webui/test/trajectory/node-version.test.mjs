...
import {
  ...
  nodeFloorMessage,
} from '../../server/trajectory/node-version.mjs';
...
// v2.1 (in-product): the plugin manifests are gone; the declarative surface is
// the webui package.json (engines) and this package's README.
...
test('the package README states the floor, range and FTS5 condition', async () => {
  const text = await readFile(path.join(PACKAGE, 'README.md'), 'utf8');
  assert.match(text, /22\.13/, 'README.md does not state the 22.13 floor');
  assert.match(text, /22\.19/, 'README.md does not state the 22.19 verified range');
  assert.match(text, /FTS5/, 'README.md does not mention the FTS5 condition');
  // And the note itself has to stay accurate about which lines lack FTS5.
  assert.match(FTS5_NOTE, /23\.x/);
});

test('the studio declares no native module, which is what removes the ABI question', async () => {
  // mcode writes the projection with better-sqlite3, a native module bound to one
  // Node ABI. The studio must not add a second, differently-bound copy: it reads the
  // same SQLite file with the runtime's own built-in driver instead.
  const source = await readFile(path.join(PACKAGE, 'server', 'trajectory', 'sqlite.mjs'), 'utf8');
