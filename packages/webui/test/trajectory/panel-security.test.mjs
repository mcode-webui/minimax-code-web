...
async function shippedWebFiles(root = path.join(import.meta.dirname, '..', '..', 'public', 'trajectory')) {
  const out = [];
...
test('the page loads no remote origin and permits no inline script', async () => {
  const html = await readFile(path.join(import.meta.dirname, '..', '..', 'public', 'trajectory', 'index.html'), 'utf8');
