...
import {
  ...
  sessionsCandidates,
  resolveDataDir,
} from '../../server/trajectory/config.mjs';
...
// v2.1 (in-product): shipped surface is the studio module dir plus its assets.
...
test('no shipped file hardcodes a path belonging to a particular machine', async () => {
  const offenders = [];
  // v2.1 (in-product): the shipped surface is the studio itself — its server
  // modules and its web assets — not the whole webui package (the webui's own
  // test fixtures legitimately contain synthetic home-directory paths).
  const SHIPPED = [
    path.join(PLUGIN, 'server', 'trajectory'),
    path.join(PLUGIN, 'public', 'trajectory'),
  ];
  const files = [];
  for (const root of SHIPPED) files.push(...await collectFiles(root, PLUGIN));
