...
| Independent anomaly REST snapshot channel | ✅ | v2.0.0 (lease B02): `server/lib/alerts.js` (203 lines) + `GET /api/alerts` REST snapshot with live `alerts.append` / `alerts.update` control frames on `/api/stream` + frontend bell icon + unread count. 3 levels (info/warn/error), 100-entry ring buffer, 60s dedup window. `lib-alerts.test.js` (17) + `routes-alerts.test.js` (7). |
...
| `token.first_run` control event | ✅ | v2.0.0 (lease C08): `server/lib/state-bus.js#pushTokenFirstRun` emits a `control` frame (`{type: "control", name: "token.first_run", data: {token, persistPath}}`) over the WebSocket event stream (`/api/stream`) to every connected tab on first boot. Replay-guarded by `auth.js#isFirstRun()` + persistent `tokenAcknowledged` flag. |
