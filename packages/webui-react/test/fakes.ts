...
export function makeHarness(opts: HarnessOptions = {}) {
  ...
  const reg: Registry = {
    ...
    workspace: {
      ...
      browse: async () => ({ dir: null, parent: null, entries: [] }),
      recents: () => [],
      addRecent: NOOP,
      listRecent: async () => [],
      listDir: async () => ({ ok: false, dir: null, parent: null, entries: [] }),
      readFile: async () => ({ ok: false, path: null, content: null }),
      createDir: async () => ({ ok: true }),
      openInSystem: async () => ({ ok: true }),
      rawFileUrl: (p: string) => '/api/fs/raw?path=' + encodeURIComponent(p),
      gitStatus: async () => ({ ok: true, isRepo: false, branch: null, files: [] }),
      gitBranches: async () => ({ ok: true, branches: [] }),
