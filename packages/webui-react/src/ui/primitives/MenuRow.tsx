 ...
 */
export function MenuRow({ icon, text, value, valueTone = 'neutral', onClick, ariaLabel, expanded }: MenuRowProps) {
  const className = 'menu-row' + (expanded ? ' menu-row--expanded' : '');
  return (
    <button type="button" className={className} aria-label={ariaLabel} aria-expanded={expanded} onClick={onClick}>
